...
export const createPassengerFlightBooking = async (req: Request, res: Response): Promise<void> => {
  // Extract data parameters seamlessly from either URL params OR request body vectors
  const passengerId = req.params.passengerId || req.body.passengerId;
  ...
  try {
    ...
    // 8. Fetch flight metadata code string to decrement memory metrics inside Memurai/Redis cache
    ...
    if (flightQuery.rows.length > 0) {
      await redis.hincrby('flight:seats', flightQuery.rows[0].flight_number, -1);
    }
